...
describe("useDebounce", () => {
  ...
  it("cancels previous timeout when deps change", () => {
    ...
    const { rerender } = renderHook(({ deps }) => useDebounce(callback, 1000, deps), {
      initialProps: { deps },
    })
  ...
  it("cancels previous timeout when delay changes", () => {
    ...
    const { rerender } = renderHook(({ delay }) => useDebounce(callback, delay), {
      initialProps: { delay },
    })
  ...
  it("uses latest callback when deps change", () => {
    ...
    const { rerender } = renderHook(({ callback, deps }) => useDebounce(callback, 1000, deps), {
